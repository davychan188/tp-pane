...
離線 GeoPackage（`.gpkg`）／Excel 樹木清單檢視器，專為園藝／樹木調查工作流而設：**地圖（或地圖 PDF）＋屬性＋PDF 頁面檢視（主）＋可選相片**。  
...
- **媒體可獨立匯入**，唔使對應地圖／樹木清單；預設「全部媒體」。檔名前綴（例如 `T1` → `T1_*.jpg`）僅作可選篩選「只顯示呢棵樹」
- **無需 GPKG** 亦可：匯入 Excel 清單＋地圖 PDF＋媒體資料夾，或 **從零加樹**（點地圖標註）
- **自動儲存**：樹木清單與標記（含 ID／x／y／DBH／H／S 等）寫入 `localStorage`，重新整理／離線 PWA 可還原；地圖檔需再匯入
- **隱藏媒體**：頂部「隱藏媒體／顯示媒體」收合右側媒體欄，方便野外只看地圖與屬性（偏好會記住）
- 樹木清單可 **內嵌編輯** ID／DBH／H／S／樹種（**點一下**欄位即可；Pencil／手指／滑鼠）
- 開發可用 `?demo=1` 深層連結載入樣本（主工具列不再顯示示範按鈕）
- 右側媒體欄以 **PDF 頁面**為主視圖（頁碼 chips、上／下頁、縮放）；相片預設收合，可按「顯示相片」開啟。PDF／相片／地圖參考可放大至約 **8×～12×**
...
3. **媒體資料夾**（可獨立於地圖／清單）  
   - 選相片／PDF 後，右側**主區顯示 PDF 頁面**（唔使先選樹，亦唔使檔名係 `T1_*`）  
   - 多個 PDF 時用頂部分頁／tabs 切換檔案；下方頁碼 chips（`#01`…）與「現在第 x / y 頁」揀頁  
   - 相片預設隱藏；可按「顯示相片」展開（仍支援左右切換／縮放）  
...
- Species／Defect／Location 等其餘欄位同樣可點一下編輯  
...
5. 左側 **樹木清單** 顯示每棵樹的 **x／y（相對檢視框 %）**；可點選、可 ✕ 刪除；無需相片亦可  
   - **點一下** **Tree ID／Species／DBH／H／S** 即可改值（不必雙擊），與左下「樹木資料」同步，並自動儲存
6. **匯出列表 Export CSV**：下載含 `Species,DBH,Height,Spread,Defect,Location` 與座標欄
...
3. 建議 **橫向（landscape）**：左約 50% 地圖＋樹木資料，右約 50% PDF 頁面（主）。不需媒體時可按 **隱藏媒體** 讓地圖／屬性佔滿。
...
7. **PDF 頁面（主視圖）**：下方頁碼 chips（`#01`、`#02`…）、**上頁／下頁**、指示「現在第 x / y 頁」；多檔時用 PDF tabs 切換。縮放最高約 **12×**；可 **新分頁開啟**。左右方向鍵亦換頁（相片區未開啟時）。
8. **相片（可選）**：預設收合；按 **顯示相片** 後可用左右掣／滑動，縮放最高約 **8×**（捏合／雙擊／拖曳）。地圖參考 PDF／圖片最高約 **8×**。
...
- 按 **媒體資料夾**（或側欄）選多個相片／PDF；支援相簿／檔案 App（Android／iOS）
- **唔使**對應地圖或樹木清單；匯入後預設以「全部媒體」顯示；右側**主區為 PDF 頁面**，相片預設收合
- 多個 PDF → tabs 選檔，再以頁碼 chips 揀頁；地圖標註／樹木清單與媒體互不阻擋
- 狀態列會顯示載入數量；0 個檔／無權限／非媒體檔會有清楚提示

### 可選：檔名前綴篩選（PhotoRenamer）
...
- 格式：`{TreeID}_{說明}.{jpg|png|webp…}`（可選，唔係匯入必要條件）
- `TreeID` 與 GPKG／Excel／屬性表的編號欄一致
- 可用同伴工具 **PhotoRenamer** 批次改名後再選入本檢視器
- 一個編號可有多張相片；開啟「顯示相片」後每次只顯示**一張直放**（非主視圖）
...
| PDF 閱讀 | 右側主區 PDF 頁面檢視＋頁碼 chips／上／下頁；相片可選顯示 |
| UI 文案 | 中英對照（繁體中文為主） |
