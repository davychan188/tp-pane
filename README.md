...
- 相關 PDF／相片／地圖參考可放大至約 **8×～12×**；開啟相關 PDF 會展開下方預覽（「收起」還原）
...
3. **媒體資料夾**（可獨立於地圖／清單）  
   - 選相片／PDF 後，右側會立即顯示**全部**已匯入媒體（唔使先選樹，亦唔使檔名係 `T1_*`）  
   - 面板可切換「全部媒體」／「只顯示呢棵樹」（有檔名前綴時先有用）  
   - 匯入嘅 PDF 會以檔案 chip 列出，點擊展開預覽
...
7. 相片操作：左右掣或左右滑動；顯示檔名與 `2 / 4`。可用 **放大／縮小／重設**（＋／−／1×，最高約 **8×**）；觸控支援捏合縮放、雙擊縮放，放大後可拖曳平移。換樹或換下一張會重設縮放。
8. 相關 PDF：號碼掣 `#07`、`#08`；點頁碼或預覽區會 **展開下方 PDF**（相片暫縮），按 **「收起」** 或 Esc 還原。預覽最高約 **12×**；可 **新分頁開啟**。地圖參考 PDF／圖片最高約 **8×**。
...
3. 按 **媒體資料夾** 載入現場相片與調查 PDF（檔案不出裝置；可獨立匯入，唔使對應地圖列表）。
...
- **唔使**對應地圖或樹木清單；匯入後預設以「全部媒體」顯示全部相片，PDF 區列出全部 PDF chip
- 地圖標註／樹木清單與媒體互不阻擋
...
| 項目 | 說明 |
|---|---|
| 佈局 | 新增右側媒體欄；橫向約 50/50；可 **隱藏相片**；媒體可獨立匯入（全部媒體／只顯示呢棵樹） |
...
| PDF 閱讀 | 相關 PDF 展開（`pdf-focus`）＋更高縮放 |
| UI 文案 | 中英對照（繁體中文為主） |
...
- 相片／相關 PDF／地圖參考支援 **放大／縮小／重設**（transform scale；相片／地圖參考約至 **8×**、相關 PDF 約至 **12×**；iframe／object 原生縮放有限故採外層 scale）
